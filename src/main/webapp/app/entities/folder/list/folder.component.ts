...
export class FolderComponent implements OnInit {
  private static readonly NOT_SORTABLE_FIELDS_AFTER_SEARCH = ['name', 'parentFolderId'];
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.folders = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IFolder[] | null): IFolder[] {
    // If there is previous link, data is a infinite scroll pagination content.
    if (this.links().prev) {
      const foldersNew = this.folders ?? [];
      if (data) {
        for (const d of data) {
          if (foldersNew.map(op => op.id).indexOf(d.id) === -1) {
            foldersNew.push(d);
          }
        }
      }
